...
- [x] **Eventos S2S y At-Least-Once Real:**
...
- [x] **Proyección de Perfil y Staleness Guard (Server-Side):**
...
- [x] **Mutación PII y Redirección Segura:**
...
- [x] **Invitaciones Asíncronas (Soft Throttling & Aceptación Segura):**
...
- [x] **Gestión de Roles (Set-Based & Step-Up Auth):**
...
- [x] **Visor de Auditoría y Partition Pruning (Prepared Statements):**
...
- [x] **Data Minimization y Redaction (HMAC Key Management):**
...
- [x] **EntitlementService Centralizado (Fail-Closed):**
...
- [x] **Test Server-Side Stale Guard:** *Request* directo al API sensible con proyección vencida devuelve consistente 403/409, no solo validación UI.
- [x] **Test DLQ Replay (S2S Idempotencia):** Reinyectar un evento ya procesado no duplica efectos, solo audita "duplicate ignored".
- [x] **Test Soft Throttling Indistinguible:** Abuso de API de invitación responde `202 Accepted`, pero silencia envíos.
- [x] **Test RBAC Step-Up:** Administrador sin MFA reciente intenta mutación de roles y es bloqueado; tras *step-up*, es permitido.
- [x] **Test HMAC Redaction Key Rotation:** Usando un nuevo `kid`, los eventos recientes se enmascaran con la nueva clave, mientras los antiguos siguen permitiendo *matching* sin exponer texto plano.
- [x] **Test Anti-Sargable con Prepared Statements:** Endpoint forense con *prepared statements* es evaluado (ej. `EXPLAIN (ANALYZE, BUFFERS)`) para confirmar que escanea las particiones correctas y no hace *seq scan* global.
- [x] **Test Billing Webhook Signature & Divergence:** Webhook sin firma válida es rechazado agresivamente. Mismo `event_id` resultando en un `outcome_hash` distinto dispara *incident hook*.
- [x] **Test Billing Drift Reconciliation:** Simular pérdida de *webhook*; el *Job* de pull restablece el estado de suscripción y *entitlements*.
- [x] **Test Strict Logging Rules (Anti-Leakage):** *Inputs* con emails/tarjetas/tokens se evalúan; aserción de que la BD de `activity_log` **solo** contiene hashes HMAC y máscaras, nunca texto en crudo.
- [x] **Test EntitlementService en Job CLI:** Un *Job* asíncrono o comando CLI detiene su ejecución en modo *Fail-Closed* si el *Tenant* no tiene *entitlement* válido.
