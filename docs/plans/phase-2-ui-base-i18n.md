...
- [x] **Package Manager Único:** `npm` es el único gestor permitido.
...
- [x] Verificar e imponer React 19 como estándar absoluto. Fijar versiones exactas de `react` y `react-dom` en `package.json`.
...
- [x] Requisito de tooling: Imponer `engines: { "node": ">=20" }` en `package.json`.
...
- [x] Inicializar y configurar `shadcn/ui`. Versionar `components.json` mediante PR dedicado.
- [x] **Política Estricta de shadcn/ui:** shadcn genera código, no es una dependencia.
...
- [x] Configurar resolve de páginas en Inertia (`resources/js/app.tsx`).
...
- [x] Implementar sistema de layouts persistentes separados (`TenantLayout` y `CentralLayout` sin estado global compartido).
- [x] Configurar `HandleInertiaRequests`.
...
- [x] **Source of Truth:** Laravel es la única fuente. El frontend consume exclusivamente archivos `lang/*.json` (JSON translations exportados).
- [x] Integrar `react-i18next`.
- [x] Precedencia de idioma: `?lang=` > `cookie` > `perfil usuario` > `default app`.
...
- [x] **Estrategia de Carga (Core vs Deferred) y Fail-Fast:**
...
- [x] **Estándar E2E Único:** Configurar Playwright (JS-first).
- [x] **Multihost & DB Strategy:**
...
- [x] Test E2E de Persistencia de Layout: Validar estado observable real (ej. abrir sidebar).
- [x] Test E2E de i18n: Navegar y refrescar validando aislamiento del locale respetando la política de host-only cookie, verificando explícitamente en el test que la cookie devuelta tiene `Path=/` y NO contiene atributo `Domain`.
